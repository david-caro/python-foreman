    ...
    def __new__(meta, cls_name, bases, attrs):
        ...
        for plugin in (pl for pl in plugins if not pl.startswith('_')):
            try:
                ...
                continue
            for mname, funcs in myplugin.DEFS.items():
...
class Foreman(with_metaclass(MetaForeman, object)):
    """
    ...
    def _get_local_defs(self, strict=True):
        ...
        for f_name in sorted(files):
            ...
            if not ver_cmp(f_ver, self.version):
                if strict:
    ...
    def _generate_api_defs(self, use_cache=True):
        ...
        resources = {}
        for name, entires in data['docs']["resources"].items():
            new_resource = ResourceMeta.__new__(ResourceMeta, str(name),
                                                (Resource,), entires)
            resources[name] = new_resource
        for name, resource in resources.items():
            for fname, ffunctions in resource._foreign_methods.items():
                for func_name, func in ffunctions.items():
